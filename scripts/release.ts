...
async function main(): Promise<void> {
  ...
  if (cmd === 'auth') {
    if (subCmd === 'login') {
      ...
      if (isErr(result)) {
        await log.error(`Login failed: ${result.error.message}`);
        process.exit(1);
      }
      process.exit(0);
    }
    ...
    if (subCmd === 'status') {
      ...
      if (isErr(result)) {
        await log.error(`Status check failed: ${result.error.message}`);
        process.exit(1);
      }
      process.exit(0);
    }
    ...
    if (subCmd === 'logout') {
      ...
      if (isErr(result)) {
        await log.error(`Logout failed: ${result.error.message}`);
        process.exit(1);
      }
      process.exit(0);
    }

    await log.error(`Unknown auth sub-command: "${subCmd}". Valid: login | status | logout`);
    printHelp();
    process.exit(1);
  }
  ...
  if (cmd === 'publish') {
    ...
    if (isErr(optsResult)) {
      await log.error(`publish: ${optsResult.error.message}`);
      printHelp();
      process.exit(1);
    }
  ...
  await log.error(`Unknown command: "${cmd}". Use --help for usage.`);
  printHelp();
  process.exit(1);
}
...
if (import.meta.main) {
  main().catch(async (e: unknown) => {
    await log.critical('Unhandled error in release CLI:', e);
    process.exit(1);
  });
}
