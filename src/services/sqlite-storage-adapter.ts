...
export class SqliteStorageAdapter implements IStorageAdapter {
    ...
    async exportData(): Promise<{products: Product[], categories: Category[], orders: Order[]}> {
        try {
            const data = await invoke<{products: Product[], categories: Category[], orders: Order[]}>('export_data')
            return data
        } catch (error) {
    ...
    async importData(data: {products: Product[], categories: Category[], orders: Order[]}): Promise<void> {
        try {
