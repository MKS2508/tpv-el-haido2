...
const OrderHistory = memo(({
    ...
    const sortedAndFilteredOrders = useMemo(() => {
        ...
        if (filterStatus !== 'all') {
            filteredOrders = filteredOrders.filter(order => order.status === filterStatus).filter(order => order.itemCount > 0)
        } else {
    ...
    useEffect(() => {
        ...
        if (currentRef && !isMobile) { // Only add wheel listener on non-mobile
            ...
            <div className={cn(
                ...
                <Select value={filterStatus} onValueChange={setFilterStatus}>
                    ...
                    <SelectContent className="bg-popover">
                        ...
                        <SelectItem value="paid"><span className="flex items-center gap-2"><Banknote
                            className="text-success"/> <span>Pagado</span></span></SelectItem>
                        <SelectItem value="unpaid"><span className="flex items-center gap-2"><HandCoins
                            className="text-destructive"/> <span>No pagado</span></span></SelectItem>
                        <SelectItem value="canceled"><span className="flex items-center gap-2"><XCircle
                            className="text-muted-foreground"/> <span>Cancelado</span></span></SelectItem>
                        <SelectItem value="inProgress"><span className="flex items-center gap-2"><Loader2
                            className="animate-spin text-info"/> <span>En progreso</span></span></SelectItem>
                    </SelectContent>
                </Select>
            </div>
                    ...
                    <Table className="border-collapse border border-border">
                        <TableHeader>
                            <TableRow className="bg-muted">
                                <TableHead className="cursor-pointer border border-border touch-manipulation"
                        ...
                        <TableBody>
                            {sortedAndFilteredOrders.map((order: Order) => (
                                <TableRow key={order.id} className="hover:bg-muted/50 touch-manipulation">
                                    ...
                                    <TableCell
                                        className="border border-border">{order.total.toFixed(2)}€</TableCell>
                                    <TableCell
                                        className="border border-border">{order.itemCount}</TableCell>
                                    <TableCell
                                        className="border border-border">{order.tableNumber === 0 ? 'Barra' : order.tableNumber}</TableCell>
                                    <TableCell className="border border-border">
                                        {order.status === 'paid' && order.paymentMethod === 'efectivo' && (
                                            <div className="flex flex-col items-center">
                                                ...
                                                <div
                                                    className="w-full border-t border-border my-1"></div>
                                    ...
                                    <TableCell className="border border-border justify-center ml-auto mr-auto">
                                        <Button variant="outline" onClick={() => handleDetails(order)}
                            ...
                            <div className={cn(
                                ...
                                <ScrollArea className={cn(
                                    ...
                                    <div className="space-y-4">
                                        <div className={cn(
                                            isMobile ? "grid grid-cols-2 gap-2" : "space-y-4"
                                        )}>
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <Input value={`${selectedOrder.total.toFixed(2)}€`} readOnly
                                                       className={cn("bg-muted font-semibold text-success", isMobile ? "h-10 text-sm" : "")}/>
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <Input
                                                    value={selectedOrder.tableNumber === 0 ? 'Barra' : selectedOrder.tableNumber.toString()}
                                                    readOnly className={cn("bg-muted", isMobile ? "h-10 text-sm" : "")}/>
                                            </div>
                                            {!isMobile && (
                                                <div>
                                        ...
                                        <div>
                                            ...
                                            {isMobile ? (
                                                ...
                                                <Table className="border-collapse border border-border mt-2">
                                                    ...
                                                    <TableBody>
                                                        {selectedOrder.items.map((item, index) => (
                                                            <TableRow key={index} className="hover:bg-muted/50 touch-manipulation">
                                                                <TableCell className="border border-border">{item.name}</TableCell>
                                                                <TableCell className="border border-border">{item.quantity}</TableCell>
                                                                <TableCell className="border border-border">{item.price.toFixed(2)}€</TableCell>
                                                                <TableCell className="border border-border">{(item.price * item.quantity).toFixed(2)}€</TableCell>
                                                            </TableRow>
                                                        ))}
                                                    </TableBody>
                                                </Table>
