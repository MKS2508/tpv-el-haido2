...
import {runThermalPrinterCommand} from "@/services/thermal-printer.service.ts";
import { StorageMode } from "@/services/storage-adapter.interface";
...
const SettingsPanel: React.FC<SettingsPanelProps> = ({ selectedUser, setSelectedUser, users, setUsers, isSidebarOpen, isDarkMode, toggleDarkMode, thermalPrinterOptions, handleThermalPrinterOptionsChange }) => {
    const { storageMode, setStorageMode, useStockImages, setUseStockImages, touchOptimizationsEnabled, setTouchOptimizationsEnabled } = useStore()
    ...
    async function handlePrintTestTicket() {
        const result = await runThermalPrinterCommand('isConnected,execute,raw(Hello World)')

        if (isErr(result)) {
            console.error('Failed to print:', result.error.code, result.error.message)
            toast({
        ...
        console.log('Printer command result:', output)
    ...
    const handleTestConnection = async () => {
        ...
        if (isErr(result)) {
            console.error('Connection test failed:', result.error.code, result.error.message)
            toast({
                title: "Error de conexion",
                description: "No se ha podido establecer la conexion. Por favor, revise los ajustes de la impresora.",
                duration: 3000,
            });
            return '';
        }
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                <TabsContent value="printing">
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <ErrorBoundary level="component" fallbackTitle="Error en configuracion de impresora">
