...
const Login = ({ users, onLogin }: LoginProps) => {
  ...
  // Define handlers with useCallback before they're used in effects
        ...
        >
          ...
          >
            <img
              src="/logo.svg"
              alt="El Haido Logo"
              className={isMobile ? 'w-10 h-10' : 'w-20 h-20 sm:w-24 sm:h-24'}
            />
          </motion.div>
          ...
          >
            ...
            <p
              className={`text-gray-700 dark:text-gray-300 mt-1 font-medium ${isMobile ? 'text-xs' : 'text-sm'}`}
            >
                ...
                >
                  {users.map((user) => (
                    <button
                      ...
                      className="user-card flex flex-col items-center cursor-pointer p-4 rounded-2xl bg-transparent border-none"
                    >
