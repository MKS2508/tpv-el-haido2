...
import {PlusIcon, FilterIcon, BeerIcon} from 'lucide-react'
...
const Products = memo(() => {
    ...
    const {users, selectedUser, setUsers, setSelectedUser, products, setProducts} = useProductsData()
                ...
                <div className="flex flex-col space-y-4">
                                ...
                                <div className="py-4 space-y-4">
                                    <div className="space-y-2">
                                        ...
                                        {availableCategories.map((category, index) => (
                                            <div key={`category-filter-${category}-${index}`} className="flex items-center space-x-2 bg-destructive/10 border border-destructive/30 rounded p-2">
                                                <Checkbox
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {availableBrands.map((brand, index) => (
                                            <div key={`brand-filter-${brand}-${index}`} className="flex items-center space-x-2">
                                                <Checkbox
                    ...
                    <div className="flex flex-wrap gap-2">
                        {selectedCategories.map((category, index) => (
                            <Badge key={`selected-category-${category}-${index}`} variant="secondary" className="px-2 py-1">
                                {category}
                                <Button
                                    variant="ghost"
                                    size="sm"
                        ...
                        {selectedBrands.map((brand, index) => (
                            <Badge key={`selected-brand-${brand}-${index}`} variant="secondary" className="px-2 py-1">
                                {brand}
                                <Button
                                    variant="ghost"
                                    size="sm"
