...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
        ...
        >
            <Card className={`h-full bg-sidebar border-sidebar-border rounded-r-3xl shadow-lg overflow-hidden ${
                isTablet ? (isSidebarOpen ? 'w-44' : 'w-14') : (isSidebarOpen ? 'w-52' : 'w-20')
            }`} style={{boxShadow: 'var(--shadow-lg)'}}>
                <CardContent className="flex flex-col h-full overflow-hidden" style={{
                    padding: isTablet ? 'calc(var(--spacing) * 1.5)' : 'calc(var(--spacing) * 2)'
                }}>
                    <div className="flex items-center justify-center" style={{marginTop: 'calc(var(--spacing) * 4)', marginBottom: 'calc(var(--spacing) * 6)'}}>
                        <img src="/logo.svg" alt="El Haido Logo" className={`${
                            isSidebarOpen 
                                ? (isTablet ? 'h-20 w-28' : 'h-24 w-32') 
                                : (isTablet ? 'h-8 w-8' : 'h-10 w-10')
                        } transition-all duration-200`}/>
                    </div>

                    {loggedUser && (
                        <div className={`flex items-center ${isSidebarOpen ? 'space-x-3' : 'justify-center'}`} style={{marginBottom: 'calc(var(--spacing) * 6)'}}>
                            <Avatar className={isSidebarOpen ? 'h-8 w-8' : 'h-6 w-6'} style={{backgroundColor: 'hsl(var(--sidebar-accent))', color: 'hsl(var(--sidebar-accent-foreground))'}}>
                                <AvatarImage src={loggedUser.profilePicture} alt={loggedUser.name}/>
                                <AvatarFallback style={{backgroundColor: 'hsl(var(--sidebar-accent))', color: 'hsl(var(--sidebar-accent-foreground))', fontSize: 'calc(var(--font-sans) * 0.875)', fontWeight: '500'}}>{loggedUser.name.charAt(0)}</AvatarFallback>
                            </Avatar>
                            <AnimatePresence>
                                {isSidebarOpen && (
                                    <motion.div
                                    ...
                                    >
                                        <p className="text-xs sm:text-sm font-medium text-sidebar-foreground whitespace-nowrap" style={{fontFamily: 'var(--font-sans)', letterSpacing: 'var(--tracking-normal)'}}>{loggedUser.name}</p>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        </div>
                    ...
                    <div className="space-y-2" style={{marginTop: 'calc(var(--spacing) * 4)'}}>
                        <div className="flex items-center justify-center" style={{padding: 'calc(var(--spacing) * 2)'}}>
                            ...
                            {isSidebarOpen && (
                                ...
                                >
                                    <button
                                        ...
                                        onClick={onLogout}
                                        style={{padding: 'calc(var(--spacing) * 2) calc(var(--spacing) * 3)', fontFamily: 'var(--font-sans)', fontWeight: '500'}}
                                    >
