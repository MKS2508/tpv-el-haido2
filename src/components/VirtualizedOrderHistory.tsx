...
const VirtualizedOrderHistory: React.FC<VirtualizedOrderHistoryProps> = ({
    ...
    const itemHeight = isMobile ? 140 : 80; // Mobile cards are taller than table rows

    // Mobile Card Row Component - Optimizado sin Framer Motion
    const MobileOrderCard = useCallback(({ order }: { order: Order }) => {
        const getStatusIcon = () => {
            switch (order.status) {
                case 'paid':
    ...
    // Desktop Table Row Component - Optimizado sin Framer Motion
    const DesktopOrderRow = useCallback(({ order }: { order: Order }) => {
        return (
            <tr 
    ...
    const Row = useCallback(({ 
        ...
        return (
            <div style={style} className={isMobile ? "px-3" : ""}>
                {isMobile ? (
                    <div className="pb-3">
    ...
    }, [orders, isMobile, MobileOrderCard, DesktopOrderRow]);
    ...
    return (
        <div className="h-full">
            ...
            {/* Virtualized List */}
            <FixedSizeList
                height={containerHeight - (isMobile ? 0 : 50)} // Account for header on desktop
                itemCount={orders.length}
                itemSize={itemHeight}
                width="100%"
                overscanCount={5} // Precargar 5 items fuera del viewport
                style={{
                    scrollbarWidth: 'thin',
                    overflowX: 'hidden'
                }}
