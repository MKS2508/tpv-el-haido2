...
export function applyTheme(theme: ThemeConfig, isDark: boolean = false) {
  ...
  if (theme.touchOptimizations) {
    root.style.setProperty('--touch-target-size', theme.touchOptimizations.buttonMinSize || '2.75rem')
...
export function createThemeFromTweakCN(tweakCNData: any): ThemeConfig {
  ...
  return {
    ...
    preview: {
      primaryColor: tweakCNData.cssVars?.light?.primary || tweakCNData.colors?.light?.primary || '#000000',
      secondaryColor: tweakCNData.cssVars?.light?.secondary || tweakCNData.colors?.light?.secondary || '#666666',
      backgroundColor: tweakCNData.cssVars?.light?.background || tweakCNData.colors?.light?.background || '#ffffff'
    }
  }
}
