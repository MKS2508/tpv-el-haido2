...
import { SidecarService } from "@/services/sidecar-service.ts";
...
function App() {
    ...
    // Initialize sidecar and app data
    useEffect(() => {
        const initializeSidecar = async () => {
            try {
                // Check if sidecar is already running
                const status = await SidecarService.getStatus()
                console.log('Sidecar status:', status)
                
                if (!status.includes('running') || !status.includes('port 3000 is open')) {
                    console.log('Starting sidecar...')
                    const result = await SidecarService.start()
                    console.log('Sidecar start result:', result)
                    
                    // Wait for sidecar to be ready
                    await SidecarService.waitForReady()
                    console.log('Sidecar is ready')
                }
            } catch (error) {
                console.error('Failed to initialize sidecar:', error)
                // Don't block app startup if sidecar fails
            }
        }

        initializeSidecar().then(() => {
            // After sidecar is ready, initialize app data
            initializeAppData()
        })
