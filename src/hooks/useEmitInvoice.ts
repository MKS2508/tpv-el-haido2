...
export function useEmitInvoice(): UseEmitInvoiceReturn {
  ...
  const { storageAdapter, orderHistory, setOrderHistory, taxRate } = useStore();

  /**
   * Actualiza un pedido en el store y en storage
   */
  const updateOrderWithAEATInfo = useCallback(
    async (order: Order, aeatInfo: OrderAEATInfo): Promise<Order> => {
    ...
    [storageAdapter, orderHistory, setOrderHistory]
  );
  ...
  const emitInvoice = useCallback(
    async (order: Order): Promise<EmitInvoiceResult> => {
      setIsEmitting(true);
      setLastError(null);
